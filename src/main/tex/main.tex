...
L'obiettivo principale è la progettazione e lo sviluppo di un database a supporto di un'applicazione desktop pensata per gli appassionati di mixology. La piattaforma si presenta come un raccoglitore digitale in cui, inizialmente, il sistema metterà a disposizione un catalogo preimpostato, contenente tutti i cocktail ufficialmente riconosciuti dall'IBA (International Bartenders Association), aggiornati fino ad una certa data, ma sarà possibile inserire nuove creazioni.\\
\\
La \textbf{parte centrale} dell'applicativo è la seguente:\\
Ogni drink, che sia un classico IBA o una creazione originale, sarà descritto in maniera precisa e comprenderà informazioni importanti sui suoi ingredienti e sulla preparazione. Il sistema terrà traccia non solo del nome e dell'eventuale utente creatore, ma anche dell'anno di ideazione, della categoria di appartenenza e di una descrizione generale.\\
Dal punto di vista pratico, verranno memorizzati gli ingredienti necessari, le garnish suggerite e i passaggi dettagliati per la preparazione, accompagnate da foto e keywords per facilitarne l'individuazione e la categorizzazione.\\
Gli utenti si interfacceranno con il sistema secondo tre livelli di accesso:\\
gli utenti  non registrati potranno esplorare il catalogo senza interagirvi: non potranno creare ricette, scrivere recensioni ...\\
Al contrario, un utente registrato diventerà parte attiva della community: potrà arricchire il database creando e pubblicando nuovi drink personalizzati. Inoltre, sarà possibile salvare i drink nella sezione \textit{"preferiti"}, aggiungere recensioni testuali alle ricette ed esportare (salvando su file) i dati di un drink, per poter condividere la ricetta.\\
Il sistema prevede la registrazione come \textbf{bar}. Ogni bar è associato ad un certo numero di \textit{utenti registrati}, i quali possono aggiungere una \textbf{creazione di gruppo}.
\\
...
Per ogni utente registrato vengono salvati nome, cognome, e-mail e password e, dopo la prima iscrizione, potrà autenticarsi accedendo con la sua e-mail e password. Un utente registrato può visualizzare i drink già presenti nel database e crearne di nuovi.\\
Un bar, esattamente come un utente singolo, può visualizzare i drink già presenti nel database e crearne di nuovi.\\
...
\begin{enumerate}
    ...
    \item Salvataggio su file dei dati dei drink;
...
Ad eccezione degli admin, tutti gli utenti registrati e i bar possono condividere ricette, solo admin e utenti registrati possono salvare ricette come "preferite", ma solo gli utenti registrati possono recensirle.
...
\hline
    ...
    recensione & R & 60.000\\
    \hline
    \footnotesize creazioneDiGruppo\normalsize & R & 5.000\\
    \hline
...
Per l'eliminazione della gerarchia su "Utente" si è deciso di applicare un collasso verso l'alto, dato il numero di attributi e la relazione in comune. Per distinguere amministratori e utenti, si è deciso di aggiungere un attributo "ruoloUtente" (amministratore/registrato), in modo tale da distinguere i due ruoli.
...
\\
Inoltre, si è scelto di usare due relazioni distinte per distinguere la creazione di una ricetta da parte di un utente singolo o di un bar, distinguendo i due concetti.
...
\textbf{creazioni} (Utenti: \underline{userID}, Drink: \underline{drinkID}, dataCreazione)\\
...
\subsection{Query SQL}
    ...
    Gli identificatori Drink.drinkID, User.userID, Bar.barID sono di tipo tipo int auto\textunderscore increment, pertanto non c'è bisogno di inserirli manualmente.
\begin{enumerate}
...
\begin{link}
    https://github.com/2menc/MixologyDB.git
\end{link}
