...
### -Di ogni drink vengono salvati i seguenti dati: nome, utente creatore (se non è un drink IBA), anno di creazione, ingredienti, decorazioni, preparazione, categoria, descrizione, foto, parole chiave utili per la ricerca. 
...
## inoltre, si consiglia, prima di avviare l'applicativo, di popolare il database con i dati di default, eseguendo le istruzioni contenute in "src/main/mysql/popolamento.sql".(l'applicativo è progettato per essere eseguito anche senza il popolamento iniziale, ma alcune funzioni potrebbero essere limitate).
